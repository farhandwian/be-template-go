...
func SetupDependencyWithDatabaseAndEmail(apiPrinter *helper.ApiPrinter, mux *http.ServeMux, jwtToken helper.JWTTokenizer, db *gorm.DB, ec EmailConfig) {

	// gateways
	generateId := gateway.ImplGenerateId() //
	generateRandom := gateway.ImplGenerateRandom()
	...
	sendEmail := gateway.ImplSendEmailUsingGmail()               // UsingGmail(model.Email(ec.MailFrom), ec.EmailSmtpHost, ec.EmailSMTPPort, ec.GmailUsername, ec.GmailAppPassword) //
	sendOTP := gateway.ImplSendOTPToWA()                         // ToWA(os.Getenv("WHATSAPP_API_URL"))                                                                                 //
	...
	userEnableUsecase := usecase.ImplUserEnable(userGetOneByID, userSave)
	...
	checkAccessToHandler := middleware.Logging(checkAccessUsecase, 0)
	...
	// controllers
	apiPrinter.
		// Add(controller.ExperimentHandler(mux)).
		Add(c.CheckAccessHandler(checkAccessToHandler)).
		...
		Add(c.UserUpdateHandler(userUpdateToHandler)).
